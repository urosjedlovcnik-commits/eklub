-- ===== PREVERJANJE IN POPRAVEK MONTH CONSTRAINT V SWIMMER_MONTHLY_FEES =====
-- Ta skript preveri constraint za month polje in ga popravi če je potrebno
...
-- 3. PREVERI ALI SO OBSTOJEČE VADNINE V 0-based FORMATU
SELECT 
    month,
    COUNT(*) as stevilo_vadnin,
    CASE 
        WHEN month >= 0 AND month <= 11 THEN '0-based (0=Januar, 11=December)'
        WHEN month >= 1 AND month <= 12 THEN '1-based (1=Januar, 12=December)'
        ELSE 'NAPACEN'
    END as format
FROM swimmer_monthly_fees
GROUP BY month
